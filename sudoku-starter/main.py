...
class Grid:
    ...
    def get_domain(self, row, col):
        """
        Get the domain of possible values for a given cell
        """
        domain = set(range(1, 10))
        for i in range(9):
            # Check the row
            if self.get_cells()[row][i] in domain:
                domain.remove(self.get_cells()[row][i])
            # Check the column
            if self.get_cells()[i][col] in domain:
                domain.remove(self.get_cells()[i][col])
        # Check the 3x3 sub-grid
        row_init = (row // 3) * 3
        col_init = (col // 3) * 3
        for i in range(row_init, row_init+3):
            for j in range(col_init, col_init+3):
                if self.get_cells()[i][j] in domain:
                    domain.remove(self.get_cells()[i][j])
        return domain
...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        # Initialize the minimum remaining value with infinity
        min_remaining_values = float('inf')
        # Initialize the selected variable with None
        selected_variable = None
        
        # Loop through each variable in the grid
        for i in range(9):
            for j in range(9):
                ...
                if len(grid.get_cells()[i][j]) > 1 :
                    ...
                    # Calculate the remaining values for the variable
                    remaining_values = len(grid.get_cells()[i][j])
                    # If the remaining values is less than the current minimum,
                    # update the minimum and select the variable
...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        # Initialize the queue with all the variables that have a value assigned
        Q = []
        Q1 = []
        for i in range(9):
            for j in range(9):
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            ...
            # remove var from all related variables
            variables_assigned_row, failure_row = self.remove_domain_row(grid, var[0], var[1])
            
            
           
           
            variables_assigned_col, failure_col = self.remove_domain_column(grid, var[0], var[1])
            variables_assigned_unit, failure_unit = self.remove_domain_unit(grid, var[0], var[1])
            
            
            
            
            
            # check if any removal resulted in a failure
            if failure_row or failure_col or failure_unit:
                
               
                return True
            
            # add all variables that had their domains reduced to size 1 to the queue
            variables_assigned = variables_assigned_row + variables_assigned_col + variables_assigned_unit
          
            for v in variables_assigned:
                if len(grid.get_cells()[v[0]][v[1]]) >= 1:
                    Q.append(v)
...
class Backtracking:
    ...
    def search(self, grid, var_selector,AC3):
            ...
            # Q = AC3.pre_process_consistency(grid)
            ...
            for d in domain:
                ...
                grid_copy.get_cells()[i][j] = d
                
                if AC3.consistency(grid_copy, [(i,j)]):
                    continue
                
                
                result = self.search(grid_copy, var_selector,AC3)
...
for p in problems:
    ...
    # print(Q)
    Q = []
    g = backtracking.search(g, MRV(9),AC3())
    
    time_end = time.time()
    time_mrv = time_end - time_start
    
    print("time=",time_mrv)
    
    running_time_mrv.append(time_mrv)
    ...
    time_start = time.time()
    AC3().pre_process_consistency(g1)
    backtracking = Backtracking()
    g1 = backtracking.search(g1, FirstAvailable(9),AC3())
    time_end = time.time()
    time_first_available = time_end - time_start
    
    print("time=",time_first_available)
    
    running_time_first_available.append(time_first_available)
...
print(max(running_time_first_available))
print(max(running_time_mrv))
